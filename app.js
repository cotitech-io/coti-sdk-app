...
(async () => {
  try {
    ...
    try {
      ...
      */
      ...
      if (e.message === 'User does not exist!') {
        // setting trust score. Only one time action.
    ...
    // WALLET and WEBSOCKET use
    const wallet = new Wallet({ seed, fullnode: fullNodeUrl, network, trustScoreNode: trustScoreNodeUrl });
    // listen for balance changes
    wallet.onBalanceChange((balanceChangedAddress) => console.log('Balance change: ', balanceChangedAddress));
    //listen for new transaction or transaction status changes
    wallet.onReceivedTransaction((receivedTransaction) => console.log('Received transaction', receivedTransaction));
    //listen for new generated address
    wallet.onGenerateAddress((addressHex) => console.log('Generated address', addressHex));

    /* initiate Address instance to load to Wallet instance.
    ...
    */
    const address = await wallet.generateAddressByIndex(addressIndex);
    // loading the addresses to wallet
    await wallet.loadAddresses([address]);
    ...
    // new address
    const newAddress = await wallet.generateAddressByIndex(newAddressIndex);
    // load the new address to wallet
    await wallet.setAddress(newAddress);
    ...
    // END OF WALLET and WEBSOCKET use
    ...
    const hardFork = await nodeUtils.isNodeSupportMultiCurrencyApis('mainnet', wallet.getFullNode());
    ...
    if(hardFork === transactionUtils.HardForks.MULTI_CURRENCY){
      ...
      await nodeUtils.getUserTokenCurrencies(userHash, wallet, wallet.getFullNode());
